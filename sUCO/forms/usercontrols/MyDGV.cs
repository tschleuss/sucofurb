            ...
            if (this.AllowDrop)
            {
                if (drgevent.Effect == DragDropEffects.Move)
                {
                    ...
                    else if (DragDropType == 1)
                    {
                        ...
                        {
                            IList<Acao> targetList = this.Fluxo.BuscarListaAcaoPorLinha(DragDropTargetIndex);
                            IList<Acao> sourceList = this.Fluxo.BuscarListaAcaoPorLinha(DragDropSourceIndex);
                            ...
                            {
                                this.Fluxo.ListaRaias[i].ListaAcoes.Insert(DragDropTargetIndex + 1, targetList[i]);
                            }

                            for (int i = 0; i < sourceList.Count; i++)
                            {
                                this.Fluxo.ListaRaias[i].ListaAcoes.RemoveAt(DragDropTargetIndex);
                                this.Fluxo.ListaRaias[i].ListaAcoes.Insert(DragDropTargetIndex, sourceList[i]);
                            }
